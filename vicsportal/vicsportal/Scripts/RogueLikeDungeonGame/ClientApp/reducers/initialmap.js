...
function initialMap(){
    ...
    for(let i =-3;i<=3;i++){
    	for(let j = 0;j<=3;j++){
    		...
    		if(j===0){
    			visibletiles.push(locs.playerlocation+i)	
    			
    			
    		}else{
    			visibletiles.push(locs.playerlocation+j*WIDTH+i)
    			visibletiles.push(locs.playerlocation-j*WIDTH+i)
    		}
    		if(3-Math.abs(i)===j){
    			break;
	...
	let  visiblemap = Object.assign([],tiles)
